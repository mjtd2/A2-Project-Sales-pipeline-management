...
public class EditStageFrame extends JDialog implements ActionListener
{
	...
	public EditStageFrame(BackendManager backendMgr, int probability, String stageName, boolean moveLeads, boolean assumeSuccessful, int stageIndex)
	{
   ...
   private void setUpGUI()
   {
		...
		stageNameTextBox = new JTextField(mStageName);
		...
		probabilitySpinner.setValue((Integer) mProbability);  
		...
		moveLeadsCheckbox.setSelected(mMoveLeads);
		...
		assumeSuccessfulCheckbox.setSelected(mAssumeSuccessful);
		getContentPane().add(assumeSuccessfulCheckbox);
		
		//Set the window's properties
		setTitle("Edit stage of negotiation");
		setResizable(false);
		setSize(585,185);
		setVisible(true);
	}
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		else if(e.getSource() == saveButton)
		{
			...
			if(backendManager.editStageOfNegotiation(mStageIndex, stageNameTextBox.getText(), probabilitySpinner.getValue(), moveLeadsCheckbox.isSelected(), assumeSuccessfulCheckbox.isSelected()))
				dispose();
		}
	}
}